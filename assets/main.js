// this is the part of array as paragraph which is give randomly paragraph
...
// hereby take querySelector as specific class with tagName

const

    typingText = document.querySelector(".textarea p"),

    inpField = document.querySelector(".box .form-control"),
...
function loadParagraph() {
    ...
    // randomly change paragraphs which is in array

    const ranIndex = Math.floor(Math.random() * paragraphs.length);

    // push in textarea class
    ...
    // when span class typing start this moment span take active class

    typingText.querySelector("span").classList.add("active");

    // this moment input focus in text for write or wrong typing text
...
function initTyping() {
    ...
    // when characters start typing this moment take time start
    ...
    if (charIndex < characters.length - 1 && timeLeft > 0) {
        ...
        // in here characters continue in limit time to stop typing when time is finish
        ...
        if (typedChar == null) {
            ...
            if (charIndex > 0) {
                ...
                // in here characters count incorrect number
            ...
            // in here character if right then add correct class
            ...
            // in here character if wrong then add incorrect class
        ...
        // in here characters remove active class
        ...
        // in here wpm decide that infinite or 0, and cannot take less time

        wpm = wpm < 0 || !wpm || wpm === Infinity ? 0 : wpm;

        // in here wpmtag print

        wpmTag.innerText = wpm;

        // in here mistaketag print

        mistakeTag.innerText = mistakes;

        // in here cmptag print
...
function initTimer() {
    ...
    // here time in not take after finish line as 0

    if (timeLeft > 0) {

        // here decrement number as 1

        timeLeft--;

        // this count as print timetag
        ...
        // in here finish then take new count
...
function resetGame() {
    ...
    // apply here load paragraphs for the reason of when game start newly

    loadParagraph();

    // clear the time in reset

    clearInterval(timer);

    timeLeft = maxTime;

    // here charIndex mistakes isTyping reset

    charIndex = mistakes = isTyping = 0;

    // field value is in empty place

    inpField.value = "";

    // timeTag reset from 60s
...
// here inputField depend on addEventListener in input typing otherwise function is not working

inpField.addEventListener("input", initTyping);

// here tryAgainBtn it also depend on add event listener otherwise function is not working
